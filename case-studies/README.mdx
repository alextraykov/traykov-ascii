...
Start from [`_template.mdx`](./_template.mdx). Its evidence-first sequence is a drafting aid, not a set of mandatory headings. Remove prompts, skip sections that do not fit the record, and do not force a neat failure arc when the source material cannot support one.
